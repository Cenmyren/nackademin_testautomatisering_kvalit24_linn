...
def test_add_product_to_catalog(page: Page):
    ...
    admin_page.admin_grid_products.first.wait_for(state="visible", timeout=20000)
    ...
    expect(admin_page.admin_grid_products).to_have_count(count_before_adding + 1, timeout=20000)
...
def test_remove_product_from_catalog(page: Page):
    ...
    # admin_page.reload()
    admin_page.create_product(new_product_name)
    admin_page.page.locator(".product-item", has_text=new_product_name).wait_for(state="visible")
    count_before_removing = admin_page.get_current_product_count()

    # Delete product
    deleted_product = admin_page.delete_product_by_name(new_product_name)

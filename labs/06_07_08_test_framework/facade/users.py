...
class UsersFacade:
    ...
    def _setup_routing(self):
        """
        Intercept requests from frontend and route them to the correct host
        so that Jenkins container can reach the backend and frontend.
        """
        ...
        # Route frontend calls if needed
        self.page.route("http://localhost/*",
                        lambda route: route.continue_(
                            url=route.request.url.replace("localhost", frontend_host)
                        ))
    ...
    def login_as_new_user(self):
        ...
        # Setup routing for Docker/Jenkins
        self._setup_routing()
